  ...
  data () {
    ...
    return {
      ...
      DefaultConfig: {
        ...
        theme: 'modern', // 默认主题
        menubar: true,
        toolbar: `styleselect | fontselect | formatselect | fontsizeselect | forecolor backcolor | bold italic underline strikethrough | insertfile link image media | table | alignleft aligncenter alignright alignjustify | outdent indent | numlist bullist | preview removeformat  hr | paste code | undo redo | fullscreen `, // 需要的工具栏
        plugins: `
            paste
            importcss
            image
    ...
    init () {
      ...
      window.tinymce.init({
            ...
            function uploadLink (file) {
              ...
              xhr.onreadystatechange = (response) => {
                if (xhr.readyState === 4 && xhr.status === 200 && xhr.responseText !== '') {
                  ...
                  callback(data.model, { text: file.name })
                  self.$emit('on-upload-complete', data)
        ...
        images_upload_handler: function (blobInfo, success, failure) {
          if (blobInfo.blob().size > self.maxSize * 1024 * 1024) {
            failure('图片不能超过' + self.maxSize + 'MB')
          }
