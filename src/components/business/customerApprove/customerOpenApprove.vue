  ...
  export default {
    ...
    methods: {
      ...
      downLoad(){        
        ...
        thiz.$tupHttp(thiz,'flow/openning/export','POST',data,{Accept: '*/*'},'',true).then(res=>{
          thiz.loading = false;
          thiz.$notify.success({
            title: '成功提示',
      ...
      detail(index, row) {
        this.gotoUrl('/frame/customerApprove/openDetail/'+row.id+'/other')
      },
      ...
      loadData(isSearch){
        ...
        let url = 'flow/openning/tasks'
        if(thiz.searchForm.status != 2){
          url = 'flow/openning/history'
        }
