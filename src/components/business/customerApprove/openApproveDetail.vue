<template>
  <div>
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item :to="{ path: '/frame/customerApprove/customerOpenApprove'}">开业申请</el-breadcrumb-item>
      <el-breadcrumb-item>开业申请审核</el-breadcrumb-item>
    </el-breadcrumb>
    <br/>
    ...
    <el-dialog title="审核通过"  width="40%" top="5%" :visible.sync="passdialogVisible">
      <el-form :rules="passFormRules" :model="passForm" ref="passForm" label-width="120px">
        <el-form-item label="下级审批人" prop="userIds" v-show="active < 3 ">
          <el-select v-model="passForm.userIds" placeholder="请选择">
            <el-option
    ...
    methods: {
      ...
      updateFwi(row){
        ...
        if(this.pass){
          ...
          if(this.active > 2){//省办审核通过就不需要指定人
            this.passForm.userIds = 'xxx'
          }
        }
        else{
      ...
      loadEmp(type){
        ...
        if(type=='area'){
          searchForm = {
            regions:[this.model.client.pCitregionoffice],
            pPositionabbrs:['RCD','RM','RD','RGM'],
            isDisable:0,
            pagingQuery:{
              pageIndex:1,
              pageSize:50
          ...
          searchForm = {
            provinces:[this.model.client.pCitprovincialoffice],
            pPositionabbrs:['PCD','PM'],
            isDisable:0,
            pagingQuery:{
              pageIndex:1,
              pageSize:50
        ...
        thiz.$tupHttp(thiz,'employee/listAllEmployees','POST',searchForm).then(res=>{
          thiz.employees = res.body.models;
        })
      },
      loadDictions(){
        let thiz = this;
