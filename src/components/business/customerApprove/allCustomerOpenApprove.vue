...
<script>
  export default {
    name:'keepAlive-customerApprove-allCustomerOpenApprove',
    data() {
      return {
        Dictions:{
          flow_audit_status:[]
    ...
    watch:{
      $route:function(val){
        if(val.path == '/frame/customerApprove/allCustomerOpenApprove'){
          if(this.total > 0){
            //有请求过数据需要重新加载列表
            this.loadData()
          }
    ...
    methods: {
      ...
      downLoad(){        
        ...
        thiz.$tupHttp(thiz,'flow/openning/export/v2','POST',data,{Accept: '*/*'},'',true).then(res=>{
          thiz.loading = false;
          thiz.$notify.success({
            title: '成功提示',
      ...
      approve(index, row) {
        this.gotoUrl('/frame/customerApprove/openApproveDetail/'+row.id)
      },  
      detail(index, row) {
        this.gotoUrl('/frame/customerApprove/openDetail/'+row.id+'/all')
      },
