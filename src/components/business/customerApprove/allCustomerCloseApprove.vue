  ...
  <div>
    <el-form  :model="searchForm" label-width="120px" class="demo-form-inline">
      ...
      <el-row :gutter="5">
        ...
        <el-col :span="8">
          <el-form-item label="DFS">
            <el-input v-model="searchForm.fs" size="mini"  placeholder="DFS编号"></el-input>
          </el-form-item>          
        </el-col> 
        <el-col :span="8">
          <el-form-item label="DM">
            <el-input v-model="searchForm.dm" size="mini"  placeholder="DM编号"></el-input>
          </el-form-item>          
        </el-col> 
      </el-row>
      <el-form-item>
    ...
    <el-table
      ...
      <el-table-column
        ...
        width="150">
        <template slot-scope="scope">
          {{scope.row.steps && scope.row.steps.length >= 1 ? (scope.row.steps[0].auditorId ? scope.row.steps[0].auditorId+'-'+scope.row.steps[0].auditorName :''):''}}
        </template> 
      </el-table-column>
      <el-table-column
        prop="step1"
        ...
        width="150">
        <template slot-scope="scope">
          {{scope.row.steps && scope.row.steps.length >= 1 ? (scope.row.steps[0].createTime ? new Date(scope.row.steps[0].createTime).format("yyyy-MM-dd hh:mm:ss"):''):''}}
        </template> 
      </el-table-column>
      <el-table-column
        prop="step1"
...
<script>
  export default {
    name:'keepAlive-customerApprove-allCustomerCloseApprove',
    data() {
      return {
        Dictions:{
          flow_audit_status:[]
    ...
    watch:{
      $route:function(val){
        if(val.path == '/frame/customerApprove/allCustomerCloseApprove'){
          if(this.total > 0){
            //有请求过数据需要重新加载列表
            this.loadData()
          }
    ...
    methods: {
      ...
      downLoad(){        
        ...
        thiz.$tupHttp(thiz,'flow/graduation/export/v2','POST',data,{Accept: '*/*'},'',true).then(res=>{
          thiz.loading = false;
          thiz.$notify.success({
            title: '成功提示',
      ...
      detail(index, row) {
        this.gotoUrl('/frame/customerApprove/closeDetail/'+row.id+'/all')
      },
