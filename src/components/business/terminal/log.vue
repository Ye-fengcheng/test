      ...
      <el-row :gutter="5">
        ...
        <el-col :span="8">
          <el-form-item label="登录平台">
            <el-select v-model="searchForm.applications" size="mini" clearable filterable multiple  placeholder="请选择">
              <el-option
                v-for="item in Dictions.login_application"
                :key="item.dictValue"
                :label="item.dictName"
  ...
  export default {
    ...
    methods: {
      ...
      downLoad(){        
        ...
        thiz.$tupHttp(thiz,'user/login/export','POST',data).then(res=>{
          thiz.loading = false;
          thiz.$notify.success({
            title: '成功提示',
      ...
      loadData(isSearch){
        ...
        thiz.$tupHttp(thiz,'user/login/list','POST',thiz.searchForm).then(res=>{
