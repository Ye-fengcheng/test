  ...
  <div class="add-vote">
    <div class="crumbs">
      <el-breadcrumb separator="/">
        <el-breadcrumb-item>
          <i class="el-icon-tickets"></i>新增投票</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    ...
    data () {
      ...
      return {
        comForm: {
          ...
          eachIdChance: '',
          eachDayChance: '',
          sameIdChange: 0, // 每天为同一选手投票次数，0为无限制
          voteNeedReg: 1, // 投票是否需要注册。1是，0否
          voteActivityId: 0, // 投票抽奖关联活动id
          matchNeedReg: 1, // 参赛是否需要注册。1是，0否
          matchChance: 0, // 参赛次数，0为无限制
          matchActivityId: 0, // 参赛抽奖关联活动ID
          matchDescLimit: 0, // 参赛说明字数限制，0为不使用
          matchImgLimit: 0 // 参赛图片数量限制，0为不使用
        },
        rules: {
          code: [
      ...
      commit () {
        this.$refs.comForm.validate((valid) => {
          if (valid) {
            ...
            this.loading = true
            this.$hsyHttp(this, 'dolls-service/vote-manage/add', 'POST', comForm).then(res => {
              this.loading = false
              this.$notify.success({
                title: '成功提示',
                message: '保存成功'
