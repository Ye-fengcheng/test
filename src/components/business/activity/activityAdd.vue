  ...
  <div class="activity-add">
    <div class="crumbs">
      <el-breadcrumb separator="/">
        <el-breadcrumb-item>
          <i class="el-icon-tickets"></i>发布活动</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    <div class="container">
      <el-form ref="form" :model="form" :rules="rules" label-width="120px" size="small">
        ...
        <el-form-item label="活动时间" required prop="beginTime">
          <el-col>
            <el-date-picker type="daterange" placeholder="请选择活动时间" start-placeholder="开始时间" end-placeholder="结束时间"
            v-model="time" value-format="yyyy-MM-dd" :editable="false" style="width:300px;"></el-date-picker>
          </el-col>
        </el-form-item>
        ...
        <el-form-item label="活动名额" required prop="quota">
          <el-input v-model="form.quota" placeholder="请输入活动名额" type="number" style="width:300px;"></el-input>
        </el-form-item>
        <el-form-item label="标题图片" required prop="imgUrl">
          <el-upload class="upload-wrapper" :action="uploadUrl" :headers="headers"
          :on-success="onSuccess" :on-remove="onRemove"
          :limit="1" :on-exceed="handleExceed" :before-upload="beforeUpload" ref="uploadImg">
        ...
        <el-form-item label="活动场次" class="special-item" prop="sessionList"><!-- required -->
        ...
        <el-form-item label="导入门店" required prop="excelUrl">
          <el-upload class="upload-wrapper" :action="uploadUrl" :headers="headers"
          :on-success="onSuccessFile" :on-remove="onRemoveFile" 
          :limit="1" :on-exceed="handleExceedFile" :before-upload="beforeUploadFile" ref="uploadExcel">
            <i class="el-icon-upload"></i>选择上传文件
          </el-upload>
        </el-form-item>
    ...
    onSubmit (form) {
      ...
      this.$refs[form].validate((valid) => {
        if (valid) {
          ...
          thiz.$hsyHttp(this, 'backend/prom/add', 'POST', thiz.form).then(res => {
            thiz.loading = false
