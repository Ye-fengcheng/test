      ...
      <el-row :gutter="5">
        <el-col :span="8">
          <el-form-item label="账户类型">
            <el-select v-model="searchForm.accountTypes" size="mini" multiple  placeholder="请选择">
              <el-option
                v-for="item in Dictions.account_type"
                :key="item.dictValue"
                :label="item.dictName"
    ...
    <el-table
      ...
      <!-- <el-table-column
        ...
        <template slot-scope="scope"> 
          {{scope.row.chargeTypeName ?scope.row.chargeTypeName +'('+scope.row.chargeType+')' :''}}
        </template>
      </el-table-column> -->
      <!-- <el-table-column
        prop="deposit"
      ...
      <el-table-column
        ...
        width="150">
        <template slot-scope="scope">
          {{$util.formatThousand(scope.row.payAmount/100,2)}} 
        </template>
      </el-table-column>
      <!-- <el-table-column
        prop="fee"
      ...
      <el-table-column
        ...
        width="150">
        <template slot-scope="scope">
          {{$util.formatThousand(scope.row.orderAmount/100,2)}}  
        </template>
      </el-table-column>
      <el-table-column
        prop="availableLimit"
        ...
        width="100">
        <template slot-scope="scope"> 
          {{$util.formatThousand(scope.row.availableLimit/100,2)}}
        </template>
      </el-table-column>
      <el-table-column
        prop="chargeType"
  ...
  export default {
    ...
    methods: {
      formatter(row, column) {
        ...
        else if(column.property =='createTime' || column.property =='auditTime' || column.property =='commitTime'){
      ...
      downLoad(){        
        ...
        thiz.$tupHttp(thiz,'account/cash/export/v1','POST',data).then(res=>{
          thiz.loading = false;
          thiz.$notify.success({
            title: '成功提示',
            message: '文件已加入下载队列，稍后前往 "基础设置-内容管理-导出下载" 下载文件'
      ...
      loadData(isSearch){
        ...
        thiz.$tupHttp(thiz,'account/cash/list/v1','POST',thiz.searchForm).then(res=>{
