    ...
    <el-form  :model="searchForm" label-width="120px" class="demo-form-inline">
      ...
      <el-row :gutter="5">
        <el-col :span="8">
          <el-form-item label="地区">            
            <tup-arae-select type="area" size="mini" v-model="searchForm.condition.areaObject"  ></tup-arae-select>
          </el-form-item>
        </el-col>
        <el-col :span="8">
      ...
      <el-form-item>
        <div style="line-height: 26px;">
          ...
          <el-button style="float:left" icon="el-icon-upload2" @click="gotoUrl('/frame/product/proportionProvinceImport')"  size="mini" type="primary">上传</el-button>
          <el-button icon="el-icon-plus" size="mini" @click="gotoUrl('/frame/product/addProportionP')" type="primary">批量创建</el-button>
...
<script>
  export default {
    ...
    watch:{
      ...
      "searchForm.condition.areaObject":function(val){
        let datas = [];
        val.map(item=>{
          if(item.code !='999')
            datas.push(item.code)
        })
        this.searchForm.condition.areaCodes = datas;
      },
    ...
    methods: {
      ...
      deleteAllProportion(){
        ...
        this.$confirm('此操作将永久删除所有记录, 是否继续?', '提示', {
          ...
          thiz.$tupHttp(thiz,'product/removeAllOrderProportionByProvince','POST').then(res=>{
            thiz.loading = false;
            thiz.loadData();
          },res=>{
            thiz.loading = false;
      ...
      downLoad(){        
        ...
        thiz.$tupHttp(thiz,'product/exportOrderProportionByProvince','POST',data,{},'',true).then(res=>{
          thiz.loading = false;
          thiz.$notify.success({
            title: '成功提示',
      ...
      treatDatas(datas){
        ...
        for(let i =1;i<datas.sheet1.length;i++){
          arr.push({
            areaCode:datas.sheet1[i][0],
            pCode:datas.sheet1[i][1],
            type:1,
            orderRatio:datas.sheet1[i][2]
          })
        }
        let thiz = this;
        this.loading = true;
        ...
        thiz.$tupHttp(thiz,'product/importOrderProportionByProvince','POST',arr).then(res=>{
          thiz.loading = false;
          if(res.body.valid){
            thiz.loadData() 
          }else{
