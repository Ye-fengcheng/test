    ...
    <el-form  :model="searchForm" label-width="120px" class="demo-form-inline">
      ...
      <el-form-item>
        <div style="line-height: 26px;">
          ...
          &nbsp;&nbsp;
          <el-button size="mini" @click="$util.winOpen('static/useJs/doc/ProductQuota.xlsx?'+$util.getRandom())" type="text">下载模板</el-button>
          ...
          <el-button style="float:left;margin-left:10px;" icon="el-icon-plus" size="mini" @click="gotoUrl('/frame/product/addProductQuota')" type="primary">批量创建</el-button>
  ...
  export default {
    ...
    methods: {
      ...
      treatDatas(datas){
        ...
        for(let i =1;i<datas.sheet0.length;i++){
          arr.push({
            ...
            quotaQty:datas.sheet0[i][1],
            mustQty:datas.sheet0[i][2],
            mqq:datas.sheet0[i][3],
            isFirstorder:datas.sheet0[i][4],
            isNonfirstorder:datas.sheet0[i][5],
            regionCodes:datas.sheet0[i][6],
            provinceCodes:datas.sheet0[i][7],
            isAccumulate:datas.sheet0[i][8]?datas.sheet0[i][8]:1,
            beginTime:datas.sheet0[i][9],
            endTime:datas.sheet0[i][10]
          })
        }
        let thiz = this;
        this.loading = true;
        thiz.$tupHttp(thiz,'inventory/uploadProductQuotaExcel','POST',arr).then(res=>{
          thiz.loading = false;
          if(res.body.valid){
            thiz.loadData() 
          }else{
      ...
      downLoad(){        
        ...
        thiz.$tupHttp(thiz,'inventory/exportProductQuotas','POST',data,{},'',true).then(res=>{
          thiz.loading = false;
          thiz.$notify.success({
            title: '成功提示',
      ...
      loadData(isSearch){
        ...
        thiz.$tupHttp(thiz,'inventory/getFmsProductQuotaRulesByPage','POST',thiz.searchForm).then(res=>{
