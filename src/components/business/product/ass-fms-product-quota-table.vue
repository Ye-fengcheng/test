  ...
  <div>
    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <div>
            {{scope.row.regionCodes == '999' ?'全国' :scope.row.regionName}}
            <el-button 
            @click="editItemRegion(scope.$index, scope.row)"
            size="mini" 
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <div>
            {{scope.row.provinceCodes == '999' ?'全国' :scope.row.provinceName}}
            <el-button 
    ...
    <el-dialog title="选择地区" width="30%" top="5%" :visible.sync="dialogAreaVisible">
      <tup-arae-store-select ref="areaSelect1" type="area" v-model="areaData1" :needstore="false"></tup-arae-store-select>
  ...
  methods: { 
    ...
    getArea(){
      ...
      if(val.isarea==='1'){
        ...
        this.products[this.itemIndex].provinceCodes = codeArray.join(',');
        this.products[this.itemIndex].provinceName = nameArray.join(',');
      }
    },
    getArea1(){
      debugger
      let val = this.$refs.areaSelect1.getValues();
      if(!val){
        val = this.areaData1;
      }
      if(val.isarea==='1'){
        ...
        this.products[this.itemIndex].regionCodes = codeArray.join(',');
        this.products[this.itemIndex].regionName = nameArray.join(',');
      }
    },
    setArea(val){
      let thiz = this;  
      let areas = this.products[val].provinceCodes.split(',');
    ...
    setArea1(val){
      let thiz = this;  
      let areas = this.products[val].regionCodes.split(',');
      let names = this.products[val].regionName.split(',');
      if(areas){
        let datas = [];
        for(let i=0;i<areas.length;i++){
          datas.push({
