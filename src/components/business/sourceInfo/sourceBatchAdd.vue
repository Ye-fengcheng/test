  ...
  <div class="sourceBatchAdd">
    <div class="crumbs">
      <el-breadcrumb separator="/">
        <el-breadcrumb-item>
          <i class="el-icon-tickets"></i>新增批次</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    <div class="" v-loading="temloading">
      <div class="sourceBatch" v-if="active < list.length">
        ...
        <div class="sourceBatchInfo">
          ...
          <div class="sourceBatchInput">
            <el-form>
              ...
              <el-form-item label="事件：">
                <el-input type="textarea" v-model="detailList.events" :rows="5" size="small" placeholder="请输入事件" style="width:50%;"></el-input>
          ...
          <el-button type="primary" size="small" @click="toNext">下一页</el-button>
      ...
      <div v-else class="sourceBatchCon">
        <el-form>
          <el-form-item label="批次号：" required>
            <el-input v-model.trim="sns" type="textarea" :rows="5" size="small" placeholder="请输入批次号" style="width:50%;"></el-input>
...
export default {
  ...
  methods: {
    ...
    toPev () {
      if (this.active < this.detailDTOList.length) {
        ...
        this.detailDTOList[this.active].templateItemId = this.list[this.active].id
      }
    ...
    toSaveBatch () {
      ...
      let batchData = {
        sns: this.sns.replace('\n', '').replace(/\s*/g, ''),
        sourceTemplateId: this.sourceTemplateId,
        detailDTOList: this.detailDTOList
      }
      this.loading = true
      this.$hsyHttp(this, 'benefit-service/product/source/info/add', 'POST', batchData).then(res => {
        this.loading = false
        this.$notify.success({
          title: '成功',
          message: '保存成功'
  ...
  created () {
    ...
    if (this.$route.query.sourceTemplateId) {
      this.sourceTemplateId = Number(this.$route.query.sourceTemplateId)
      this._getDetail(this.sourceTemplateId)
    }
  }
}
</script>
