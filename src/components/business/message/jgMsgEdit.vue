  ...
  <div class="activity-add">
    ...
    <div class="container">
      <el-form ref="form" :model="form" :rules="rules" label-width="130px" size="small">
        <el-row :gutter="5">
          <el-col :span="8">
            <el-form-item label="推送平台" required>
              <el-select v-model="form.platform" size="mini" filterable placeholder="请选择">
        ...
        <el-row :gutter="5">
          <el-col :span="24" v-show="form.platform == 1">
    ...
    <el-dialog title="选择地区" width="30%" top="5%" @opened="handleOpen" :visible.sync="areaDialogVisible">
    ...
    <el-dialog title="选择地区" width="30%" top="5%" @opened="handleOpen1" :visible.sync="areaDialogVisible1">
      <tup-arae-store-select ref="areaSelect1" v-model="areaData1" :needstore="false"></tup-arae-store-select>
      <div slot="footer" class="dialog-footer">
...
export default {
  ...
  methods: {
    ...
    handleOpen(){
      this.$refs.areaSelect.cheeckNode();
    },
    handleOpen1(){
      this.$refs.areaSelect1.cheeckNode();
    },
        ...
        if (valid) {
          ...
          thiz.$tupHttp(thiz, url, 'POST', thiz.form).then(res => {
            ...
            if (res.body.valid) {
              ...
              thiz.errorTableData = res.body.extra ? (res.body.extra.errorManageUsersList ? res.body.extra.errorManageUsersList :res.body.extra.errorBusinessUsersList):[]
              thiz.errDialogVisible = true
            }
          }, res => {
            thiz.loading = false
    ...
    getType () {
      ...
      this.$tupHttp(thiz, 'manage/pushMessage/getValidPushMessageType', 'GET').then(res => {
        ...
        thiz.getDetail(thiz.$route.query.id)
      })
    },
    getDetail (id) {
      let thiz = this;
      this.$tupHttp(this, 'manage/pushMessage/getPushMessageById?pushMessageId=' + id, 'POST').then(res => {
        ...
        if (this.form.platform == 1) {
          if(this.form.userType =='all' || this.form.userType =='defined'){
            this.menu = this.form.userType
          }else {
            this.menu = 'area'
            let arr = this.form.userType.split(',')
            this.areaData = {
              isarea:'1',
              data:[],
              codes:[]
            ...
            arr.map(item=>{
              this.areaData.data.push({code:item})
              this.areaData.codes.push(item)
            })
          }        
        }
        if (this.form.platform == 2) {
          if(this.form.userType =='all' || this.form.userType =='defined'){
            this.menu1 = this.form.userType
          }else {
            this.menu1 = 'area'
            this.areaData1 = {
              isarea:'1',
            ...
            arr.map(item=>{
              this.areaData1.data.push({code:item})
              this.areaData1.codes.push(item)
            })
          }          
        }
        if(this.form.attachmentName1){
          this.fileList.push({
            name: this.form.attachmentName1,
            url: this.form.attachment1,
            size: 0
          });
        }
        if(this.form.attachmentName2){
          this.fileList.push({
            name: this.form.attachmentName2,
            url: this.form.attachment2,
            size: 0
          });
        }
        if(this.form.attachmentName3){
          this.fileList.push({
            name: this.form.attachmentName3,
            url: this.form.attachment3,
            size: 0
          });
        }
        if(this.form.type){
          let hasItem = this.typeDatas.some(item=>{
            return item.id == thiz.form.type
          })
          if(!hasItem) this.form.type =''
        }
      })
    },
  },
