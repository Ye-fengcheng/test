  ...
  <div>
    <el-form  :model="searchForm" label-width="120px" class="demo-form-inline">
      ...
      <el-form-item>
        ...
        <el-button icon="el-icon-plus" size="mini" @click="gotoUrl('/frame/order/backendOrder/tzmOrder')" type="primary">添加</el-button>
        <el-button icon="el-icon-plus" size="mini" @click="gotoUrl('/frame/order/backendOrder/tzmOrderImport')" type="primary">导入数据</el-button>
    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope"> 
          {{$util.formatThousand(scope.row.adjustPayable,2)}}
        </template>
      </el-table-column>
      <el-table-column
        prop="productPay"
      ...
      <el-table-column
        prop="totalDiscountDiffMoney"
        label="使用折扣差金额"
        width="120">
        <template slot-scope="scope"> 
          {{$util.formatThousand(scope.row.totalDiscountDiffMoney,2)}}
        </template>
      </el-table-column>
      <el-table-column
        prop="discountValue"
...
<script>
  export default {
    name:'keepAlive-order-backendOrder-tzmOrderList',
    data() {
      return {
        multipleSelection:[],
        Dictions:{
    ...
    watch:{
      $route:function(val){
        if(val.path == '/frame/order/backendOrder/tzmOrderList'){
          if(this.total > 0){
            //有请求过数据需要重新加载列表
            this.loadData()
          }
    ...
    methods: {
      ...
      edit(ind,row){
        this.gotoUrl('/frame/order/backendOrder/backTzOrderEdit/'+row.id+'/TZ/0')
      },
      downLoad(){        
        let thiz = this;
        thiz.loading = true;
