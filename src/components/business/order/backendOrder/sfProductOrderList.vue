  ...
  <div>
    <el-form  :model="searchForm" label-width="120px" class="demo-form-inline">
      ...
      <el-form-item>
        ...
        <el-button icon="el-icon-plus" size="mini" @click="gotoUrl('/frame/order/backendOrder/sfProductOrder')" type="primary">添加</el-button>
        <el-button icon="el-icon-plus" size="mini" @click="gotoUrl('/frame/order/backendOrder/sfProductOrderImport')" type="primary">导入数据</el-button>
...
<script>
  export default {
    name:'keepAlive-order-backendOrder-sfProductOrderList',
    data() {
      return {
        multipleSelection:[],
        Dictions:{
    ...
    watch:{
      $route:function(val){
        if(val.path == '/frame/order/backendOrder/sfProductOrderList'){
          if(this.total > 0){
            //有请求过数据需要重新加载列表
            this.loadData()
          }
