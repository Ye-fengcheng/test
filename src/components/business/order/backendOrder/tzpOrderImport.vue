<template>
  <div>
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item :to="{ path: '/frame/order/backendOrder/tzpOrderList'}">正调整单</el-breadcrumb-item>
      <el-breadcrumb-item>批量导入正调整单</el-breadcrumb-item>
    </el-breadcrumb>
    <br/>
    <div style="line-height: 26px;">
      <el-row>
        <el-col :span="18">
          ...
          &nbsp;&nbsp;
          <el-button size="mini" @click="$util.winOpen('static/useJs/doc/PBsOrder.xlsx?'+$util.getRandom())" type="text">下载模板</el-button>
        </el-col>
  ...
  export default {
    ...
    methods: {
      ...
      treatDatas(datas,file){
        ...
        for(let i =1;i<datas.sheet0.length;i++){
          if(datas.sheet0[i][1]){
            arr.push({
              ...
              deliverWithOrder:'',
              groupFlag:'',
              oldOrderNo:datas.sheet0[i][6],
              orderDetailRemark:datas.sheet0[i][8],
              orderRemark:datas.sheet0[i][7],
              orderType:'TZ',
              pCode:datas.sheet0[i][2],
              plusOrMinusAdjust:1,
              projectName:'',
              deliverQty:datas.sheet0[i][3],
              adjustPayable:datas.sheet0[i][5],
              unitPrice:datas.sheet0[i][4],
              status:0,
            }) 
          }              
        }
        let thiz = this;
        this.loading = true;
        thiz.$tupHttp(thiz,'order/importDrafts','POST',{draftList:arr,remark:thiz.remark,orderType:'TZ',plusOrMinusAdjust:1}).then(res=>{
          thiz.loadData()
          thiz.$notify.success({
            title: '成功提示',
            message: '操作成功'
      ...
      detail(index, row){
        this.gotoUrl('/frame/order/backendOrder/tzpOrderImportList/'+row.id)   
      },
      downLoad(index, row) {
