<template>
  <div>
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item :to="{ path: '/frame/order/backendOrder/tzmOrderList'}">负调整单</el-breadcrumb-item>
      <el-breadcrumb-item>批量导入负调整单</el-breadcrumb-item>
    </el-breadcrumb>
    <br/>
    <div style="line-height: 26px;">
      <el-row>
        <el-col :span="18">
          ...
          &nbsp;&nbsp;
          <el-button size="mini" @click="$util.winOpen('static/useJs/doc/MinusBsOrder.xlsx?'+$util.getRandom())" type="text">下载模板</el-button>
        </el-col>
  ...
  export default {
    ...
    methods: {
      ...
      treatDatas(datas,file){
        ...
        let thiz = this;
        this.loading = true;
        thiz.$tupHttp(thiz,'order/importDrafts','POST',{draftList:arr,remark:thiz.remark,orderType:'TZ',plusOrMinusAdjust:0}).then(res=>{
          thiz.loadData()
          thiz.$notify.success({
            title: '成功提示',
            message: '操作成功'
      ...
      detail(index, row){
        this.gotoUrl('/frame/order/backendOrder/tzmOrderImportList/'+row.id)   
      },
      downLoad(index, row) {
