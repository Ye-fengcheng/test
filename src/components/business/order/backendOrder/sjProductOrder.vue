<template>
  <div>
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>批量创建SJ内部申请表</el-breadcrumb-item>
    </el-breadcrumb>
    <br/>
    <div>
      <el-form ref="comForm" label-width="160px">
        <div class="grid-content bg-purple">
          <fms-backend-table v-model="comForm.datas" type="SJ" type-name="SJ-内部申请表"></fms-backend-table>
        </div> 
      </el-form>
    </div>
  ...
  export default {
    ...
    methods: {
      ...
      commit(){        
        ...
        thiz.$tupHttp(thiz,'order/saveDrafts','POST',{draftList:data,orderType:'SJ',plusOrMinusAdjust:''}).then(res=>{
          thiz.loading = false;
          if(res.body.valid){
            thiz.$notify.success({
              title: '提示',
