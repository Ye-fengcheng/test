<template>
  <div>
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>批量创建正调整单</el-breadcrumb-item>
    </el-breadcrumb>
    <br/>
    <div>
      <el-form ref="comForm" label-width="160px">
        <div class="grid-content bg-purple">
          <fms-backend-table v-model="comForm.datas" type="TZ" type-name="正调整单" :adjust="1"></fms-backend-table>
        </div> 
      </el-form>
    </div>
...
<script>
  import fmsBackendTable from '@/components/business/order/ass-fms-backend-tz-table'
  export default {
    data() {
      return {        
        comForm:{
    ...
    methods: {
      ...
      commit(){        
        ...
        thiz.$tupHttp(thiz,'order/saveDrafts','POST',{draftList:data,orderType:'TZ',plusOrMinusAdjust:1}).then(res=>{
          thiz.loading = false;
          if(res.body.valid){
            thiz.$notify.success({
              title: '提示',
