  ...
  <div>
    ...
    <div>
      <el-form :model="model" :rules="modelRules" ref="modelForm" label-width="220px">
        ...
        <el-row :gutter="5">
          <el-col :span="24">
            <el-form-item label="发货地址">
              <el-input size="mini" v-model="model.shipAddress" placeholder="请输入内容"></el-input>
            </el-form-item>
          </el-col>
        </el-row> -->
        ...
        <fms-backend-table v-model="model.details" @delete="handleDelete" :clientId="model.clientId" :type="type" :adjust="adjust" type-name="后台订单"></fms-backend-table>
      </el-form>
    </div>
    <el-dialog
      title="选择客户"
...
<script>
  import fmsBackendTable from '@/components/business/order/ass-fms-backend-tz-edit-table'
  export default {
    data() {
      return {
        id:this.$route.params.id,
        type:this.$route.params.type,
        adjust:this.$route.params.adjust,
        visible:false,
        clientVisible:false,
        parentVisible:false,
        productData:[],
    ...
    methods: {
      ...
      getBackName(){
        switch(this.$route.params.adjust){
          case '1':
            this.backName = '正调整单'
            this.backPath = '/frame/order/backendOrder/tzpOrderList'
          break;
          case '0':
            this.backName = '负调整单'
            this.backPath = '/frame/order/backendOrder/tzmOrderList'
          break;
        }
      },
      submitDraft(){
      ...
      loadDictions(){
        let thiz = this;
        let params = ["outlet_status","order_project_type","order_apply_type"];
