  ...
  <div>
    <el-form  :model="searchForm" label-width="120px" class="demo-form-inline">
      ...
      <el-form-item>
        ...
        <el-button icon="el-icon-plus" size="mini" @click="gotoUrl('/frame/order/backendOrder/tzpOrder')" type="primary">添加</el-button>
        <el-button icon="el-icon-plus" size="mini" @click="gotoUrl('/frame/order/backendOrder/tzpOrderImport')" type="primary">导入数据</el-button>
...
<script>
  export default {
    name:'keepAlive-order-backendOrder-tzpOrderList',
    data() {
      return {
        multipleSelection:[],
        Dictions:{
    ...
    watch:{
      $route:function(val){
        if(val.path == '/frame/order/backendOrder/tzpOrderList'){
          if(this.total > 0){
            //有请求过数据需要重新加载列表
            this.loadData()
          }          
    ...
    methods: {
      ...
      edit(ind,row){
        this.gotoUrl('/frame/order/backendOrder/backTzOrderEdit/'+row.id+'/TZ/1')
      },
      downLoad(){        
        let thiz = this;
        thiz.loading = true;
