  ...
  <div>
    <el-form  :model="searchForm" label-width="120px" class="demo-form-inline">
      ...
      <el-form-item>
        ...
        <el-button icon="el-icon-plus" size="mini" @click="gotoUrl('/frame/order/backendOrder/srProductOrder')" type="primary">添加</el-button>
        <el-button icon="el-icon-plus" size="mini" @click="gotoUrl('/frame/order/backendOrder/srProductOrderImport')" type="primary">导入数据</el-button>
...
<script>
  export default {
    name:'keepAlive-order-backendOrder-srProductOrderList',
    data() {
      return {
        multipleSelection:[],
        Dictions:{
    ...
    watch:{
      $route:function(val){
        if(val.path == '/frame/order/backendOrder/srProductOrderList'){
          if(this.total > 0){
            //有请求过数据需要重新加载列表
            this.loadData()
          }
    ...
    methods: {
      ...
      formatter(row, column) {
        ...
        if(column.property =='projectName'){
          ...
          this.Dictions.bsorder_declaration_type.map(item=>{
            if(row[column.property]==item.dictValue){
              txt = item.dictName
            }
          })  
      ...
      edit(ind,row){
        this.gotoUrl('/frame/order/backendOrder/backOrderEdit/'+row.id+'/SR')
      },
      downLoad(){        
        let thiz = this;
        thiz.loading = true;
      ...
      loadDictions(){
        let thiz = this;
        let params = ["bsorder_draft_status","bsorder_declaration_type","order_apply_type","order_project_type"];
