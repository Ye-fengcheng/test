  ...
  <div>
    <el-form  :model="searchForm" label-width="120px" class="demo-form-inline">
      ...
      <el-form-item>
        ...
        <el-button icon="el-icon-plus" size="mini" @click="gotoUrl('/frame/order/backendOrder/sbProductOrder')" type="primary">添加</el-button>
        <el-button icon="el-icon-plus" size="mini" @click="gotoUrl('/frame/order/backendOrder/sbProductOrderImport')" type="primary">导入数据</el-button>
...
<script>
  export default {
    name:'keepAlive-order-backendOrder-sbProductOrderList',
    data() {
      return {
        multipleSelection:[],
        Dictions:{
    ...
    watch:{
      $route:function(val){
        if(val.path == '/frame/order/backendOrder/sbProductOrderList'){
          if(this.total > 0){
            //有请求过数据需要重新加载列表
            this.loadData()
          }
    ...
    methods: {
      ...
      edit(ind,row){
        this.gotoUrl('/frame/order/backendOrder/backOrderEdit/'+row.id+'/SB')
      },
      downLoad(){        
        let thiz = this;
        thiz.loading = true;
