  ...
  <div>
    <el-form  :model="searchForm" label-width="140px" class="demo-form-inline">
      ...
      <el-row :gutter="5">
        ...
        <el-col :span="8">          
          <el-form-item label="是否等价交换">
            <el-select v-model="searchForm.isEqualExchange" size="mini" clearable filterable  placeholder="请选择">
    ...
    <el-dialog title="审核"  width="25%" top="15%" :visible.sync="dialogVisible">
      <el-form :model="comForm" ref="comForm" label-width="60px">
        <el-form-item label="通过" prop="status">
          <el-switch v-model="comForm.status" :active-value="11" :inactive-value="6"></el-switch>
        </el-form-item> 
        <el-form-item label="理由" v-if="comForm.status==6" prop="reason">
...
<script>
  export default {
    name:'keepAlive-order-customerServiceApproveB',
    data() {
      return {
        Dictions:{
          declaration_status:[],
    ...
    watch:{
      $route:function(val){
        if(val.path == '/frame/order/customerServiceApproveB'){
          if(this.total > 0){
            //有请求过数据需要重新加载列表
            this.loadData()
          }
    ...
    methods: {
      ...
      detail(index, row) {
        this.gotoUrl('/frame/order/csOrderDetailSeniorCS/'+row.id)
      },
