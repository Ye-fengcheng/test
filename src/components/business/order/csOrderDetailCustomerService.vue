<template>
  <div>
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item :to="{ path: '/frame/order/customerServiceApproveL'}">售后申请制单客服审核</el-breadcrumb-item>
        ...
        <div>
          ...
          <el-form :model="model" ref="modelForm" label-width="180px">            
            <el-row>
              <el-col :span="8">
                <el-form-item label="申报产品编号/零配件编号：">
                  {{model.expressDeclarationProductNo}}
                </el-form-item>
              </el-col>
              <el-col :span="10">
                <el-form-item label="申报产品名称/零配件名称：">
              ...
              <el-col :span="6">
                <el-form-item label="申报数量：">
                  {{model.expressDeclarationQuantity}}
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
              <el-col :span="8">
                <el-form-item label="申报单位：">
                  {{model.expressDeclarationUnit}}
                </el-form-item>
              </el-col>
              <el-col :span="16">
                <el-form-item label="情况说明：">
                  {{model.expressDeclarationReason}}
                </el-form-item>
              </el-col>
            </el-row> 
          </el-form>
          ...
          <el-form :model="model" v-if="model.type != 1" ref="modelForm" label-width="180px">            
            <el-row>
              <el-col :span="8">
                <el-form-item label="申报产品编号/零配件编号：">
                  <el-input-product-code 
                    v-model="model.serviceDeclarationProductNo"
                    :param="model"
              ...
              <el-col :span="8">
                <el-form-item label="申报数量：">
                  <el-input-number 
                    v-model="model.serviceDeclarationQuantity"
                    :min="0" 
                    size="mini"
            ...
            <el-row>
              <el-col :span="8">
                <el-form-item label="申报单位：">
                  <el-input 
                    v-model="model.serviceDeclarationUnit"
                    size="mini"
                    label="描述文字"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="16">
                <el-form-item label="情况说明：">
                  <el-input 
                    v-model="model.serviceDeclarationReason"
                    size="mini"
                    label="描述文字"></el-input>
                </el-form-item>
              </el-col>
          ...
          <el-form :model="model" ref="modelForm"  v-if="model.type != 1" label-width="180px">
            <el-row>
              <el-col :span="24">
                <el-form-item label="" label-width="101px">
                  <el-radio  v-model="model.isEqualExchange" :label="0">按照申报内容更换产品</el-radio >
                  <el-radio  v-model="model.isEqualExchange" :label="1">等价交换</el-radio >
                </el-form-item>
              </el-col>
            </el-row>
            <el-row style="margin-left:101px" v-if="model.isEqualExchange == 1">
              <el-col :span="24">
                <el-button style="float:left" icon="el-icon-plus"  @click="addProduct(1)" size="mini" >添加</el-button>
                <br/><br/>
                <el-table
                  :data="exchangeProductVos"
                  style="width: 99%;"
                  border
                  height="200"
                  ...
                  <el-table-column
                    ...
                    <template slot-scope="scope">
                      <el-input-product-code 
                        v-model="scope.row.supplyProductNo"
                        :param="scope"
                  ...
                  <el-table-column
                    ...
                    <template slot-scope="scope">
                      <el-input-number 
                        v-model="scope.row.supplyNumber"
                        :min="1" 
                        size="mini"
                        label="描述文字"></el-input-number>
                    </template>
            ...
            <el-row style="margin-top:15px;">
              <el-col :span="24">
                <el-form-item label="送货对象：">
                  <el-input-customer v-model="customer" type="ALL" ></el-input-customer>
                </el-form-item>
                <el-form-item label="送货地址：">
                  <el-input v-model="model.sendAddress" disabled></el-input>
                </el-form-item>
              </el-col>
            </el-row>
          </el-form>
            ...
            <el-row>
              ...
              <!-- <el-col :span="8">
                <el-form-item label="发货时间：">
                  {{formatter({backDeliveryTime:model.backDeliveryTime},{property:'backDeliveryTime'})}}
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
              <el-col :span="8">
                <el-form-item label="预计到货时间：">
                  {{formatter({backReachTime:model.backReachTime},{property:'backReachTime'})}}
                </el-form-item>
              </el-col> -->
              <el-col :span="8">
    ...
    <el-dialog title="审核"  width="25%" top="15%" :visible.sync="dialogVisible">
      <el-form :model="comForm" ref="comForm" label-width="60px">
        <el-form-item label="通过" prop="status">
            <el-switch v-model="comForm.status" :active-value="7" :inactive-value="5"></el-switch>
          </el-form-item> 
        <el-form-item label="理由" v-if="comForm.status == 5" prop="reason">
  ...
  export default {
    ...
    methods: {
      deleteProduct(ind,row){
        this.exchangeProductVos.splice(ind,1);
      },
      handleSelectService(product,scope){     
        scope.serviceDeclarationProductName = product.pName
      },
      handleSelect(product,scope){     
        scope.row.supplyProductName = product.pName
        scope.row.supplyUnit = product.orderUnit
        scope.row.supplyPrice = product.price
      },
      addProduct(num){
        if(!num){
          num = 1;
      ...
      saveData(){
        ...
        data.push({
          ...
          serviceDeclarationProductNo:this.model.serviceDeclarationProductNo,
          serviceDeclarationProductName:this.model.serviceDeclarationProductName,
          serviceDeclarationQuantity:this.model.serviceDeclarationQuantity,
          serviceDeclarationUnit:this.model.serviceDeclarationUnit,
          serviceDeclarationReason:this.model.serviceDeclarationReason,
          isEqualExchange:this.model.isEqualExchange,
          sendTarget:this.model.sendTarget,
          sendAddress:this.model.sendAddress,
          exchangeProductVos:this.model.isEqualExchange == 1 ? this.exchangeProductVos:[]
        })
        thiz.$tupHttp(thiz,'declaration/updateDeclaration','POST',data).then(res=>{
          thiz.loading = false;
          thiz.$notify.success({
            title: '成功提示',
            message: res.body.message ? res.body.message : '操作成功'
          });
          thiz.dialogVisible = false;
          thiz.gotoUrl('/frame/order/customerServiceApproveC');
        },res=>{
          thiz.loading = false;
        })
      },
      ...
      getInfo(){
        ...
        thiz.$tupHttp(thiz,'declaration/getDeclaration?id='+this.id,'GET').then(res=>{
          let model = res.body.model;
          if(model.exchangeProductVos)
          {
            thiz.exchangeProductVos = model.exchangeProductVos;  
          }
          if(!model.isEqualExchange){
            model.isEqualExchange = 0
          }
          if(!model.photos){
            model.photos = [];
          }
          if(!model.serviceDeclarationProductNo){
            model.serviceDeclarationProductNo = model.expressDeclarationProductNo ? model.expressDeclarationProductNo : model.declarationProductNo
          }
          if(!model.serviceDeclarationProductName){
            model.serviceDeclarationProductName = model.expressDeclarationProductName ? model.expressDeclarationProductName : model.declarationProductName
          }
          if(!model.serviceDeclarationQuantity){
            model.serviceDeclarationQuantity = model.expressDeclarationQuantity ? model.expressDeclarationQuantity : model.declarationQuantity
          }
          if(!model.serviceDeclarationUnit){
            model.serviceDeclarationUnit = model.expressDeclarationUnit ? model.expressDeclarationUnit : model.declarationUnit
          }
          if(!model.serviceDeclarationReason){
            model.serviceDeclarationReason = model.expressDeclarationReason ? model.expressDeclarationReason : model.description
          }
          thiz.model = model; 
