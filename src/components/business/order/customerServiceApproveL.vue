  ...
  <div>
        ...
        <el-col :span="8">
          <el-form-item label="申报类型">
            <el-select v-model="searchForm.typeList" multiple size="mini" clearable filterable  placeholder="请选择">
              <el-option label="多发" value="1"></el-option>
              <el-option label="错发" value="2"></el-option>
              <el-option label="漏发" value="3"></el-option>
            </el-select>
          </el-form-item>
        </el-col>   
      </el-row>
    ...
    <el-dialog title="审核"  width="25%" top="15%" :visible.sync="dialogVisible">
      <el-form :model="comForm" ref="comForm" label-width="60px">
        ...
        <el-form-item label="理由" prop="reason" v-if="comForm.status==4">
...
<script>
  export default {
    ...
    watch:{
      $route:function(val){
        if(val.path == '/frame/order/customerServiceApproveL'){
          if(this.total > 0){
            //有请求过数据需要重新加载列表
            this.loadData()
          } 
    ...
    methods: {
      ...
      detail(index, row) {
        this.gotoUrl('/frame/order/csOrderDetailLogistics/'+row.id+'/other')
      },
