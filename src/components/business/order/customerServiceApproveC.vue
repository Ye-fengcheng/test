<template>
  <div>
      ...
      <el-row :gutter="5">        
        ...
        <el-col :span="8">
          <el-form-item label="审核状态">
            <el-select v-model="searchForm.statusList" multiple size="mini" filterable  placeholder="请选择">
              <el-option label="物流已审核" :value="3"></el-option>
              <el-option label="待客服修改" :value="6"></el-option>
              <el-option label="未能生成订单" :value="9"></el-option>
            </el-select>
          </el-form-item>
        </el-col>
      </el-row>
      <el-form-item>
        <el-button icon="el-icon-import" @click="dialogVisible = true" :disabled="multipleSelection.length == 0" :loading="loading" size="mini" type="success">批量审核</el-button>
        ...
        <el-button icon="el-icon-check" @click="sDialogVisible = true" :disabled="multipleSelection.length == 0" :loading="loading" size="mini" type="success">特殊处理</el-button>
        <el-button icon="el-icon-check" @click="aDialogVisible = true" :disabled="multipleSelection.length == 0" :loading="loading" size="mini" type="primary">批量更换申报产品</el-button>
      </el-form-item>
    </el-form>
    <el-table
      :data="tableData"
    ...
    <el-dialog title="特殊处理"  width="25%" top="15%" :visible.sync="sDialogVisible">
      <el-form :model="specialComForm" ref="comForm" label-width="60px">
        <el-form-item label="原因" prop="remark">
          <el-select v-model="specialComForm.reason" size="mini" clearable filterable  placeholder="请选择">
            <el-option
              v-for="item in Dictions.declaration_special_reason"
              :key="item.dictValue"
              :label="item.dictName"
              :value="item.dictValue">
            </el-option>
        ...
        <el-form-item label="备注" prop="remark">
          <el-input v-model="specialComForm.remark"  placeholder="备注"></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" size="mini" :loading="loading" @click="ssaveData">确认</el-button>
        <el-button size="mini" @click="sDialogVisible = false">取 消</el-button>
      </div>
    </el-dialog>
    <el-dialog title="批量更改申报产品" width="85%" top="5%" :visible.sync="aDialogVisible">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" size="mini" :disabled="this.exchangeProductVos.length == 0" :loading="loading" @click="saveChangeData">确认</el-button>
        <el-button size="mini" @click="aDialogVisible = false">取 消</el-button>
      </div>
    </el-dialog>
...
<script>
  export default {
    name:'keepAlive-order-customerServiceApproveC',
    data() {
      return {
        exchangeProductVos:[],
        Dictions:{
    ...
    watch:{
      $route:function(val){
        if(val.path == '/frame/order/customerServiceApproveC'){
          if(this.total > 0){
            //有请求过数据需要重新加载列表
            this.loadData()
          }
    ...
    methods: {
      ...
      ssaveData(){
        ...
        this.multipleSelection.map(item=>{
          data.push({
            id:item.id,
            remark:this.specialComForm.remark,
            reason:this.specialComForm.reason,
            status:10
          })
        })
        thiz.$tupHttp(thiz,'declaration/specialHandle','POST',data).then(res=>{
          thiz.loading = false;
          thiz.$notify.success({
            title: '成功提示',
            message: '操作成功'
          });
          thiz.sDialogVisible = false;
          thiz.loadData();
        },res=>{
          thiz.loading = false;
        })
      ...
      saveChangeData(){
        ...
        this.multipleSelection.map(item=>{
          data.push({
            id:item.id,
            exchangeProductVos:this.exchangeProductVos
          })
        })
        thiz.$tupHttp(thiz,'declaration/updateDeclaration','POST',data).then(res=>{
          ...
          thiz.exchangeProductVos = []
          thiz.aDialogVisible = false;
          thiz.loadData();
        },res=>{
          thiz.loading = false;
        })
      ...
      saveData(){
        ...
        this.multipleSelection.map(item=>{
          data.push({
            ...
            exchangeProductVos:item.equalExchangeProduct ? JSON.parse(item.equalExchangeProduct) : [],
            isEqualExchange:item.isEqualExchange,
            serviceDeclarationProductNo:item.expressDeclarationProductNo,
            serviceDeclarationProductName:item.expressDeclarationProductName,
            serviceDeclarationQuantity:item.expressDeclarationQuantity,
            serviceDeclarationUnit:item.expressDeclarationUnit,
            serviceDeclarationReason:item.expressDeclarationReason
          })
        })
        thiz.$tupHttp(thiz,'declaration/updateDeclaration','POST',data).then(res=>{
          thiz.loading = false;
      ...
      detail(index, row) {
        this.gotoUrl('/frame/order/csOrderDetailCustomerService/'+row.id)
      },
      ...
      loadDictions(){
        let thiz = this;
        let params = ["declaration_status","declaration_special_reason","declaration_type","declaration_reject_reason"];
