<template>
  <div>
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>售后申请管理</el-breadcrumb-item>
      <el-breadcrumb-item>批量创建其他售后申请</el-breadcrumb-item>
    </el-breadcrumb>
    <br/>
    <div>
      <el-form ref="comForm" label-width="160px">
        <div class="grid-content bg-purple">
          <fms-cs-table v-model="comForm.datas" type="CS" type-name="SR"></fms-cs-table>
        </div> 
      </el-form>
    </div>
...
<script>
  import fmsCsTable from '@/components/business/order/ass-fms-cs-table'
  export default {
    data() {
      return {        
        comForm:{
    ...
    methods: {
      ...
      commit(){        
        ...
        data.map(item=>{
          item.exchangeProductVos = [];
          item.exchangeProductVos.push({
            "supplyNumber":  item.shopCartNum,
            "supplyPrice":  item.price,
            "supplyProductName": item.pName,
            "supplyProductNo": item.pCode,
            "supplyUnit": item.orderUnit
          })
        })
        thiz.$tupHttp(thiz,'declaration/importOtherDeclaration','POST',data).then(res=>{
          thiz.loading = false;
          if(res.body.valid){
            thiz.$notify.success({
              title: '提示',
